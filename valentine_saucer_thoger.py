"""
Valentine Saucers - Thøgers saucer runs this code
...
MQTT_CLIENT_ID      = "thoger"
MQTT_BROKER         = "eac7a3b1df514170b0cf7e6d64162653.s2.eu.hivemq.cloud"
MQTT_USER           = "thoger" #Emilie´s cup runs this code
...
#Function for lightning animation
def rgb(r=1,g=1,b=1):
...
def sub_based_LED(topic, msg):
    ...
    if topic == Temp_TOPIC_to_T:
        print(msg)
        global loopcounter
        loopcounter = 10
        
    if topic == Color_TOPIC_to_T:
        print(msg)
        global loopcounter
        loopcounter = 1
...
client.subscribe(Color_TOPIC_to_T)
client.subscribe(Temp_TOPIC_to_T)

#Functional Loop begins here
loopcounter = 0
...
#Measure capasitance from sensor 1
    ...
    if touch1.read() <= 110:
        ...
        if colorpicker >= 6:
            ...
            client.publish(Color_TOPIC_to_E, message)
        if colorpicker == 5:
...
#Play LED animation if temperature measurements exeedes theshold
    ...
    if temperature >= 780:
        ...
        client.publish(Temp_TOPIC_to_E, message)
        loopcounter = 10
        chosencolor = LOC[colorpicker]
...
#Set new colors for next loop
    if loopcounter <= 0: #color to dim white if loopcounter is below 0
        chosencolor = LOC[6]
